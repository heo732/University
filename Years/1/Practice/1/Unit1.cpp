...
//---------------------------------------------------------------------------
//Повертає факторіал переданого параметра
...
//---------------------------------------------------------------------------
//Повертає значення рекурсивно обчисленої суми
...
void __fastcall Tfrm::btnDoClick(TObject *Sender)
{
  ...
  if(IsInputNaturalValue(edtN->Text.c_str()))//Якщо введено натуральне число
    if(StrToInt(edtN->Text.c_str()) >= 1 && StrToInt(edtN->Text.c_str()) <= 15){
      n = StrToInt(edtN->Text);
      edtRes->Text = FloatToStr(Sum(1, 1, 1, n));
    }
    else
      ShowMessage("Введіть натуральне число в діапазоні [1; 15]");      
  else
    ShowMessage("Введіть натуральне число!");
}
//---------------------------------------------------------------------------