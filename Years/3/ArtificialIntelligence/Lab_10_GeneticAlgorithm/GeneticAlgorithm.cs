...
namespace WpfProject
{
    public class GeneticAlgorithm
    {
        ...
        public GeneticAlgorithm(int populationSize, int numberGenesInChromosome,
            ...
            double borderLeft, double borderRight, FitnessFunction fitnessFunction)
        {
            if (populationSize < 1)
            ...
            if (borderRight <= borderLeft)
                throw new ArgumentOutOfRangeException("borderLeft", "Left border must be straight less than right border.");
        ...
        private void RecalculateAdaptability()
        {
            ...
            {
                adaptability.Add(fitnessFunction(borderLeft + (double)item));
            }
        }

        public double GetPhenotypeOfMaxAdaptedPerson()
        {
            return population[adaptability.IndexOf(adaptability.Max())] + borderLeft;
        }
        ...
        private void CreateStartPopulation()
        {
            ...
            int len = (int)(borderRight - borderLeft);
            for (int i = 0; i < populationSize; i++)
            {
                ulong newValue = (ulong)random.Next(len);
